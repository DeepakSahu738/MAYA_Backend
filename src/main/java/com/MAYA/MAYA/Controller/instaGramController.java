...
public class instaGramController {
    ...
    @PostMapping("/insta_prime")
    private ResponseEntity<Map<String, ResponsePOJOInstaCombinedPOJO>> generateContentIdea(@RequestBody CombinedInstaDTO request) {
        //we are doing the LangChain stuff in the service section

        String sessionId = UUID.randomUUID().toString();
        ResponsePOJOInstaCombinedPOJO CombinedPOJO = new ResponsePOJOInstaCombinedPOJO();
...
//
//            storageService.storeContentIdea(sessionId,generateCoreIdea.getContentIdeas().get(0).getContentIdea());
//            System.out.println("content idea ye h ->"+generateCoreIdea);
//            if (contentIdea == null) {
...
//            }
            ...
            CombinedPOJO.setContentIdeaList(generateCoreIdea);
            CombinedPOJO.setCaptionList(generateCaptionWithCTA);
            CombinedPOJO.setDesignAndAestheticList(generateDesignAestheticSugestion);
            CombinedPOJO.setHashtagsList(generatedHashtags1);
            CombinedPOJO.setEngagementStrategiesList(generateEngagementTips);
            CombinedPOJO.setPostingTimeList(generateBestPostingTime);

            Map<String, ResponsePOJOInstaCombinedPOJO> response = new HashMap<>();
            response.put("contentIdeaData", CombinedPOJO);
