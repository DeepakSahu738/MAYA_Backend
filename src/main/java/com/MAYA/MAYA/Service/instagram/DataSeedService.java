 ...
 * 
 * Logic:
 * - If fitlife_by_meera exists with 300+ posts → SKIP (instant)
 * - If partial/missing → DELETE demo data → batch INSERT fresh from JSON
 * - Runs in background thread so app starts immediately (Cloud Run health check passes)
 */
@Service
...
public class DataSeedService implements CommandLineRunner {
    ...
    private void seed() {
        ...
        // Quick exit: if demo is fully seeded, skip instantly
        ...
        if (existing.isPresent()) {
            ...
            // Partial data — wipe and re-seed
            log.info("Partial demo data found ({} posts). Wiping and re-seeding...", postCount);
            wipeDemoData(existing.get().getId());
            creatorRepository.delete(existing.get());
        }

        // Load JSON data (only fitlife_by_meera)
        ...
        if (postsArray == null || !postsArray.isArray() || postsArray.isEmpty()) {
            log.error("No posts found in fitlife_by_meera data file");
            return;
        }
        ...
        log.info("Created demo creator: {}", DEMO_USERNAME);
        ...
        // Compute analytics
        log.info("Computing analytics for demo creator...");
        try {
            analyticsProcessingService.processCreatorAnalytics(creator);
            log.info("Analytics computed!");
        } catch (Exception e) {
            log.warn("Analytics processing failed: {}", e.getMessage());
        }
    }
    ...
    private void wipeDemoData(Long creatorId) {
        ...
        log.info("Wiped partial demo data for creator {}", creatorId);
    }
